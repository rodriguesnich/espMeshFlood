...
**Scenario**: Single node sends message to direct neighbor

**Steps**:
1. Node A creates message with TTL=5, relay_count=0
2. Broadcasts via ESP-NOW
3. Node A's application receives its own message (loopback delivery)

**Expected**:
- sent_messages[0].ttl == 5
- app_received_count == 1
...
**Scenario**: Message propagates through one intermediate node

**Steps**:
1. Node A sends message (TTL=5, relay_count=0)
2. Node B receives and relays
3. Verify TTL decremented and relay_count incremented

**Expected**:
- B's retransmitted message: ttl=4
...
### CT-03: Multi-Hop Propagation

**Scenario**: Chain topology A → B → C → D → E

**Steps**:
1. A sends (TTL=5, relay_count=0)
2. B relays (TTL=4, relay_count=1)
3. C relays (TTL=3, relay_count=2)
4. D relays (TTL=2, relay_count=3)
5. E receives (TTL=2, relay_count=3)

**Expected**:
- message_id consistent across all hops
- TTL decremented: 5 → 4 → 3 → 2 → 1
...
**Scenario**: Verify messages stop propagating when TTL reaches 0

**Steps**:
1. A sends with TTL=2
2. B receives and relays (TTL→1)
3. C receives and relays (TTL→0)
4. D should NOT retransmit (TTL=0)
5. E never receives the message

**Expected**:
- D: sent_messages.count == 0
- E: sent_messages.count == 0

**Status**: ✅ PASS

---

### CT-05: Deduplication (Basic)

**Scenario**: Duplicate message IDs are rejected

**Steps**:
1. Send message M once: app receives 1 copy
2. Simulate receiving M again (same message_id)
3. Verify app only called once

**Expected**:
- app_callback called 1 time (duplicate ignored)
- cache.size() == 1
...
### CT-06: Redundant Paths (Deduplication in Diamond Topology)

**Scenario**: Message arrives via two paths simultaneously
...
**Steps**:
1. A sends message M
2. B and C both receive and relay M
3. D receives M from both B and C paths
4. D should only process one copy

**Expected**:
- D app_callback called 1 time
- D sent_messages.count == 1 (or 0, depending on TTL)
- Second copy ignored due to dedup cache

**Status**: ✅ PASS

---

### CT-10: Serialization (Protobuf Round-Trip)

**Scenario**: Message survives serialize/deserialize cycle

**Steps**:
1. Create MeshMessage with all fields populated
2. Serialize to buffer
3. Deserialize back to MeshMessage
4. Verify all fields match

**Expected**:
- All fields preserved exactly
- Multiple round-trips succeed
...
- [ ] **Clock Sync**: Beacon-based time synchronization
- [ ] **Fragmentation**: Large message support via chunking
