...
TEST_F(MessageCacheTest, SingleMessageNotInCache) {
    ...
    EXPECT_FALSE(cache->exists(sender_id, msg_id));
}

TEST_F(MessageCacheTest, AddMessageThenExists) {
    uint32_t sender_id = 1;
    uint32_t msg_id = 12345;
    cache->add(sender_id, msg_id, 0);
    EXPECT_TRUE(cache->exists(sender_id, msg_id));
}
...
TEST_F(MessageCacheTest, MultipleMessagesTracked) {
    ...
    EXPECT_TRUE(cache->exists(1, 100));
    EXPECT_TRUE(cache->exists(1, 200));
    EXPECT_TRUE(cache->exists(1, 300));
    EXPECT_FALSE(cache->exists(1, 400));
}
...
TEST_F(MessageCacheTest, ExpiredEntriesRemoved) {
    ...
    cache->add(sender_id, msg_id, current_time);
    EXPECT_TRUE(cache->exists(sender_id, msg_id));
...
TEST_F(MessageCacheTest, CacheBoundedSize) {
    ...
    small_cache.add(1, 100, 0);
    small_cache.add(1, 200, 0);
    small_cache.add(1, 300, 0);
    EXPECT_EQ(small_cache.size(), 3);
    
    // Add one more - should evict oldest
    small_cache.add(1, 400, 0);
    EXPECT_EQ(small_cache.size(), 3);
    
    // First entry should be evicted
    EXPECT_FALSE(small_cache.exists(1, 100));
    EXPECT_TRUE(small_cache.exists(1, 200));
    EXPECT_TRUE(small_cache.exists(1, 300));
    EXPECT_TRUE(small_cache.exists(1, 400));
}
...
TEST_F(MessageCacheTest, ClearRemovesAllEntries) {
    ...
    EXPECT_FALSE(cache->exists(1, 100));
    EXPECT_FALSE(cache->exists(1, 200));
    EXPECT_FALSE(cache->exists(1, 300));
}
...
TEST_F(MessageCacheTest, MixedExpirationAndCapacity) {
    ...
    mixed_cache.add(1, 100, 0);
    mixed_cache.add(1, 200, 0);
    mixed_cache.add(1, 300, 0);
    mixed_cache.add(1, 400, 0);
    mixed_cache.add(1, 500, 0);
...
TEST_F(MessageCacheTest, SameMessageIdDifferentSendersTrackedSeparately) {
    cache->add(1, 12345, 0);
    cache->add(2, 12345, 0);

    EXPECT_TRUE(cache->exists(1, 12345));
    EXPECT_TRUE(cache->exists(2, 12345));
